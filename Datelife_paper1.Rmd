...
fig_workflow_all_functions <- "Vignette Fig. 1"
...
When organisms are preserved in a fossil form, a time frame of taxon origin can
be obtained directly from the age of rock strata.
However, not all organisms fossilize well or at all.
Fossilization success alone is highly circumstancial, and varies depending on a
number of parameters including the nature of the habitat, population size, species
range breadth and physical characteristics of the organism, all of which greatly
vary across different organisms and through time.
<!--fossil recovery is also highly circumstancial-->
Thus, relying only on the fossil record to obtain a time frame of lineage divergence
<!--taxon ages--> for all life is<!--, alas,--> not possible.

Another caveat is that we usually cannot have a point age estimates of lineage origin
because of the nature of the fossilization process: when a fossil first appears
on the fossil record it is not necessarily because the lineage just originated,
it might just be bc the conditions for fossilization started at that point but the
organism might have been around for a long time before that. Because of this, we
use ranges from the age of strata where fossils are found. But this does not solve
the problem. The organism could even have been around previously, but not fossilize
under the conditions prevailing around the time of formation of rock strata below
its first appearance in the fossil record. In this sense, fossil ages can only be
considered as minimum time of origin of lineages.
<!--D Bapst work?-->


Another source that has been widely used to inform about the timing of lineage origin
is the relative rate of DNA or aminoacid substitution.
It is estimated from hypothesis of character homology (alignments) for reconstructing
phylogenetic relationships.
<!--describe the two components of rel subst rate? bc they contain the information
on absolute rate of substitution and absolute time.-->
Molecular dating techniques use external data such as absolute time calibrations
(e.g., fossils, geologic events) or absolute substitution rates <!--obtained in
the lab?? birds-->to <!--dissociate rel subs rates components and--> generate dated
phylogenies (chronograms) which contain information on absolute times of node divergence
and taxon ages. <!--With molecular dating methods, an external calibration point
is still necessary to estimate the absolute time of origin. Hence, the accumulation
of data on fossils for calibration is also continual-->
<!--This method has been particularly useful to obtain time frame of origin of organisms
with poor or no fossil record, but is also largely used for organisms with rich
fossil records-->
<!--Models allow to take into account the nature of the fossil record: Given the
characteristics of the fossilization process, the quality of dates inferred directly
from it is not always considered the best.-->
...
With this inspiration, a prototype DateLife service was developed over a series
of phylotastic hackathons [__CITE__] at the National Evolutionary Synthesis Center.
NSF funding allowed for further development leading to this paper. A core goal of
DateLife is openness of both the data sources and the code underlying the analyses,
so scientific community can take advantage of this tool to leverage available information
to the advancement of discoveries in biology.
...
-->
<!--
While its node-time search is very powerful, using available data to infer divergence
time from lineages that have never been sequenced before, its timetree option is
less powerful, their main flaw being that it does not provide source data.
However, their primary database is not fully open, giving information on a per node
basis. So, it is not possible to easily access information from original studies
for a group of taxa (One would need to perform a pair of taxa search for all pairs
from the group of interest).
Divergence time estimates for all taxa in the TToL are precomputed and stored in
the TimeTree database
All searches allow species names and higher group names. Common names are allowed
in all searches but a list of taxa.
one-taxon search was not working for dog, though.
1) ttol one lineage: it gives the evolutionary timeline of a lineage, showing the
age of main divergence events occuring along the lineage evolutionary history, i.e.,
when it shared a comon ancestor with all main lineages from which it diverged a
long time ago.
It only shows results graphically on the website.
2) ttol parwise search: allows common names, species names and higher group names
    how it handles names missing from tree: it looks in the NCBI taxonomy for the
    closest relative of the species or higher taxa. Then uses the closest relative
    to get the lineage pair age from ttol.
3) ttol group of taxa
4) ttol list of taxa: Cannot get it to work; does not work with cat, dog, turtle
search, felidae, hominidae, phrynops search, some_ants species name search. What
kind of files does it accepts: txt, rtf, doc, docx?
-->

<!--
retrieves information takes available data on lineage ages and phylogenetic relationships
to construct a tree of all life scaled to absolute time: a chronogram of all life,
while allowing the general public to search for age data within the tree for one
taxon of interest (timeline option), a pair of taxa (node time option), a group
of taxa (it can only be a single name of a taxonomic group), or a list of taxa of
interest (timetree option for the two latter).
Good things:
Allows to get ages of species with no data, by extrapolating from closely related
species with data.
Allows comparison of timelines to geologic events, such as asteroid impacts, oxygen
and carbon dioxide levels, solar luminosity.
Not so good things:
Only allows to download some data from original sources if performing a pair-of-taxa
search.
Only allows to see age ranges or confidence intervals in graphical form, when hovering
over nodes. These CIs come from ttol. It does not have CIs from original studies.
Node hovering is activated on one-taxon evolutionary history, and group-of-taxa
searches (we could not get to work list-of-taxa search). Pair-of-taxa search shows
the confidence interval of the only ttol node that constitutes the taxon-pair mrca.
You can only download data from pair-of-taxa and group-of-taxa search. It generates
a table with ages from original studies and a tree pruned from ttol, respectively.
In both cases it gives single point age estimates. We cannot download CI information
from ttol.
-->
...
DateLife can summarize this information in several formats that can be easily inspected
by users.
This allows rapidly obtaining a time frame of lineage divergence for a wide number
of taxa.
DateLife can also generate chronograms for taxa with little available information,
by using the available data as calibration points.
...
In here we present the first release of DateLife.
It contains an improved database of chronograms, more methods to summarize trees,
and new functions to visualize data.
It also allows comparison of summary methods.
...
```{r, echo = FALSE}
fig_workflow_main_foot <- paste("Stylized DateLife workflow. Detailed steps are shown in",  fig_workflow_all_functions)
```
...
`DateLife`'s chronogram database is currently built from Open Tree of Life (OToL)'s [@Hinchliff2015] tree repository. Among currently existing repositories (e.g., TreeBAse, Dryad), OToL's metadata rich tree store  is the only one meeting the requirements for proper/accurate automatized handling of trees.
Input taxon names accepted by `DateLife` are binomial species names or clades. <!--common names (implemented yet?)--> Taxon searches are performed at the species level, so when input names correspond to higher clades, `DateLife` pulls all accepted species names within the
clade from OToL's reference taxonomy to perform the search.
Input names belonging to subspecies or any other infraspecific category are treated as
species.
...
which corrects potentially misspelled names and typos, and standardizes variation in spelling
and synonyms <!--to a taxonomic reference of users choosing, degfault to otol taxonomy -->[@Boyle2013], increasing the probability to correctly find the queried
taxa in `DateLife`'s chronogram database.

Source chronogram summary tree can be assembled using the Super Distance Matrix (SDM) supertree construction approach [@Criscuolo2006] or using the median of branch lengths and the hierarchical clustering method <!-- of ttol-->. <!--Also, I just discovered the median gives non ultrametric trees... but I thonk it is fixed now, check it!).--> 
...
To apply calibrations to a tree, congruification method described in [@Eastman2013] to find shared nodes between trees (congruent nodes).
...
repository of the paper at (<https://github.com/LunaSare/datelife_paper1>) as well as in the package vignettes <!-- Dryad also? since it is all in github already-->
...
knitr::include_graphics("fig1.pdf", dpi = 360)
```
